"""这个模块实现了gammatone filter和一个滤波过程
...
TODO:
   ...
   Hohmann, V., Frequency analysis and synthesis using a Gammatone filterbank, Acta Acustica, Vol 88 (2002), 433--442
"""
...
def erb_count(centerfrequency):
    """
    centerfrequency : 中心频率 Hz
    Returns: 返回到中心频率的等效矩形频带计数。
    """
...
def erb_aud(centerfrequency):
    """
    返回听觉滤波器的等效矩形带宽,Implements Equation 13 in [Hohmann2002]_.
    :param centerfrequency: 所需的听觉滤波器的中心频率，以赫兹为单位。
    :return: 听觉滤波器在“中心频率”处的等效矩形带宽。
    """
...
def frequencies_gammatone_bank(start_band, end_band, norm_freq, density):
    ...
    :return: centerfrequency_array
    """

    norm_erb = hertz_to_erbscale(norm_freq)
    centerfrequencies = erbscale_to_hertz(arange(start_band, end_band, density) + norm_erb)
...
def design_filter(sample_rate=44100, order=4, centerfrequency=1000.0, band_width=None, band_width_factor=1.0,
    ...
    """Returns filter coefficient of a gammatone filter [Hohmann2002]_.
