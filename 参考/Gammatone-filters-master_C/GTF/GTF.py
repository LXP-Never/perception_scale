...
class GTF:
    """Python接口，用于使用C编写的全极gammatone过滤器，Python版本的过滤器也包括在内"""

    _package_dir = os.path.dirname(os.path.abspath(__file__))
    _lib_fpath = os.path.join("./GTF", 'libGTF.so')
    _c_fpath = os.path.join("./GTF", 'GTF.c')
    ...
    def filter_py(self, x, align_env=False, align_tfs=False, delay_common=None):
        """Filters in Python
        Args:
    ...
    def filter(self, x, is_env_aligned=False, is_fine_aligned=False,
        ...
        # ensure x is 2-D array
        x_len = x.shape[0]  # 语音长度
        if len(x.shape) == 1:
            x = np.reshape(x, [-1, 1])
        n_chann = x.shape[1]    # 通道数
